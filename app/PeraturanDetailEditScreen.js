...
const PeraturanDetailEditScreen = ({navigation, route}) => {
  ...
  const aturan = route.params.aturan
  ...
  const updatePeraturan = () => {
    ...
    .then(({data}) => {
      ...
      if (data.error.msg == '') {
        goBack()
        navigation.replace('PeraturanDetail', {dataRumah: dataRumah})
      }
      setIsLoading(false)
    }).catch((e) => {
      console.log(e, 'error update peraturan rumah')
    })
  }
    ...
    <KeyboardAwareScrollView>
      <View style={styles.container} >
        ...
        { role == 'Penghuni' ?
          ...
          :
            <>
              <TouchableOpacity style={{ alignItems: 'center' ,backgroundColor: '#FFB700', padding: 5, width: '35%', borderRadius: 7, marginTop: 20 }} onPress={() => updatePeraturan()}>
...
export default PeraturanDetailEditScreen;