...
const CreateBroadcastScreen = ({navigation, route}) => {
  ...
  const createBroadcast = () => {
    ...
    if (pesan == '') {
      ...
      error = true
    } else if (pesan.length > 225) {
      setPesanError('Maksimal 225 karakter')
      error = true
    } else {
      setPesanError('')
    }
      ...
      <View style={styles.container} >
        ...
        <View style={{ alignItems: 'center', justifyContent: 'center', width: '90%' }} >
          ...
          <View style={{ flexDirection: 'row', justifyContent: pesanError ? 'space-between' : 'flex-end', width: '100%' }}>
            ...
            <Text style={{ alignSelf: 'flex-end', color: 'black', fontSize: 15, fontFamily: 'PlusJakartaSans-Regular' }} >{pesan.length}/225</Text>
          </View>
        </View>
